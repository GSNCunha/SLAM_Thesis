...
class BaseController(Node):
    ...
    def update_odometry(self):
        ...
        if comm_l != COMM_SUCCESS or comm_r != COMM_SUCCESS:
            return # Skip this loop if read fails

        # Converter para inteiro de 32 bits com sinal (para lidar com números negativos no encoder)
        if p_l > 2147483647: p_l -= 4294967296
        if p_r > 2147483647: p_r -= 4294967296

        now = self.get_clock().now()
        dt = (now - self.last_time).nanoseconds / 1e9

        if self.last_l_ticks is not None and dt > 0:
            # 2. Calculate raw difference
            delta_ticks_l = p_l - self.last_l_ticks
            delta_ticks_r = p_r - self.last_r_ticks
            
            # --- FILTRO ANTITELETRANSPORTE (OVERFLOW) ---
            # Se o pulo for maior que 10.000 ticks de uma vez, é erro de leitura.
            if abs(delta_ticks_l) > 10000 or abs(delta_ticks_r) > 10000:
                self.get_logger().warn(f"Salto irreal no encoder! Ignorando leitura. L:{delta_ticks_l} R:{delta_ticks_r}")
            else:
                # Calculate displacement (Right inverted on chassis)
                d_l = (delta_ticks_l) / TICKS_PER_REV * (2 * math.pi * R)
                d_r = -(delta_ticks_r) / TICKS_PER_REV * (2 * math.pi * R)
                ...
                # --- NOVO: Cálculo das velocidades (TWIST) para o SLAM ---
                v_linear = dS / dt
                v_angular = dTh / dt
                ...
                # 5. Publish Odometry Message (Agora COMPLETA)
                odom = Odometry()
